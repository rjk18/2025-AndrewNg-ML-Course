...
def plot_learning_curve(X, y, Xval, yval, l, model_type="线性"):
    ...
    # 动态生成文件名
    if model_type == "线性":
        filename = 'output\\5_偏差方差\\5_1_线性学习曲线.png'
    else:
        filename = f'output\\5_偏差方差\\5_1_多项式学习曲线_lambda_{l}.png'
...
def plot_poly_fit(X, y, X_poly_final, train_mean, train_std, power, l=0, show_learning_curve=False):
    ...
    if show_learning_curve:
        ...
        plt.tight_layout()
        filename = f'output\\5_偏差方差\\5_1_多项式拟合与学习曲线_lambda_{l}.png'
        ...
        filename = f'output\\5_偏差方差\\5_1_多项式拟合_lambda_{l}.png'
...
plt.tight_layout()
plt.savefig('output\\5_偏差方差\\5_1_不同lambda拟合效果对比.png')
plt.show()
